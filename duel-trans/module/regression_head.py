...
class RegressionHead(nn.Module):
    ...
    def _upsample(self, x: NestedTensor, disp_pred: Tensor, occ_pred: Tensor,mask: Tensor, scale: float):
        """
        ...
        # disp_pred=up_flow.reshape(N,1, 8*H, 8*W)
        # disp_pred = F.interpolate(disp_pred, size=(h, w), mode='nearest')
        ...
        up_flow = torch.sum(mask * up_flow, dim=2)
        up_flow = up_flow.permute(0, 1, 4, 2, 5, 3)
        disp_pred=up_flow.reshape(N,1, 4*H, 4*W)
        disp_pred = F.interpolate(disp_pred, size=(h, w), mode='nearest')
        occ_pred = F.interpolate(occ_pred[:, None, ], size=(h, w), mode='nearest')  # N x 1 x H x W
    ...
    def forward(self, attn_weight: Tensor, x: NestedTensor,feat1: Tensor):
        """
